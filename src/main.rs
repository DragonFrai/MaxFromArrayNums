mod rng;

use crate::rng::Rng;
use std::time::{Instant, SystemTime, UNIX_EPOCH};

fn main() {
    let mut arr_src = gen_rand_array(1000);
    let mut arr = arr_src.clone();

    // Измеряем время сортировки
    let start = Instant::now();
    let num = make_max_mum(&mut arr);
    let total = Instant::now().duration_since(start);

    println!("Time: {:?}", &total);
    println!("Source: {:?}", &arr_src);
    println!("Number: {}", num);
}

// Генерация массива случайных чисел
fn gen_rand_array(len: usize) -> Vec<i32> {
    let mut rng = Rng::new(SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs());
    let mut arr = Vec::new();
    for _i in 0..len {
        arr.push(rng.gen_i32_in(0, 999));
    }
    arr
}

// Создает наибольшее число из массива чисел  в диапазоне [0; 999]
fn make_max_mum(arr: &mut [i32]) -> String {
    let mut str = String::new();
    arr.sort_by(|a, b| measure(*b).cmp(&measure(*a)));
    for x in arr {
        str += &format!("{}", x);
    }
    str
}

// Функция вычисления "меры" приоритета числа для постановки в начало
// 9 -> 999
// 78 -> 788
// 942 -> 942
fn measure(n: i32) -> i32 {
    if n < 10 { 100*n + 10*n + n }
    else if n < 100 { 10*n + n%10 }
    else if n < 1000 { n }
    else { unreachable!() }
}

